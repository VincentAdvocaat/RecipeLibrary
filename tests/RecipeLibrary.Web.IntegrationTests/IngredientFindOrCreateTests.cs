...
public sealed class IngredientFindOrCreateTests
{
    ...
    [Fact]
    public async Task FindOrCreateAsync_NormalizesRegionalLanguageTag_ToCatalogCode()
    {
        await using var connection = new SqliteConnection("DataSource=:memory:");
        await connection.OpenAsync();
        ...
        var repo = new EfIngredientRepository(db);
        var first = await repo.FindOrCreateAsync("nl", "tomaat", "tomaat", null, null);
        var second = await repo.FindOrCreateAsync("nl-NL", "tomaat", "tomaat", null, null);

        Assert.Equal(first.Id, second.Id);
        Assert.Equal(1, await db.IngredientTranslations.CountAsync());
        Assert.Equal("nl", await db.IngredientTranslations.Select(x => x.LanguageCode).SingleAsync());
    }
}
