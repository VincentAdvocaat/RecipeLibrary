...
public sealed class ToggleShoppingListItemCommandHandlerTests
{
    ...
    [Fact]
    public async Task HandleAsync_DoesNotTouchPantry()
    {
        var itemId = Guid.NewGuid();
        var repo = new FakeShoppingListRepository
        {
        ...
        await sut.HandleAsync(new ToggleShoppingListItemCommand { ItemId = itemId, IsChecked = true });

        // Toggle no longer depends on IPantryRepository — constructing without pantry is the assertion.
        Assert.Equal(itemId, repo.LastToggledItemId);
    }
